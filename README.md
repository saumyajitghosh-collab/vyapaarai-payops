# PayOps by VyapaarAI — market-validation prototype
...
The **Import your data** section lets a merchant upload a real bank statement and bill list — CSV directly, XLSX via the SheetJS CDN library, and **bank PDFs via pdf.js (also CDN-loaded, still fully in-browser)**: dates, narrations, debit/credit columns with CR/DR markers, "-" blanks, Indian digit grouping, wrapped two-line transactions and opening-balance seeding of the balance-reconciliation logic are all handled. Column mapping is auto-detected (HDFC/SBI/ICICI-style exports work) and correctable in the UI. The same deterministic rules then run on the merchant's own rows: invoice number in narration, customer + exact amount, part payment, short payment (≤₹10), duplicates within 3 days, probable matches (exact amount, no name) and unmatched credits become exceptions — never guesses. Results: a "₹X found needing attention" headline, an import-quality summary, exception/probable/overdue tables and a CSV download. **Everything runs in the browser; no data is uploaded anywhere.** Press "Use sample" on both cards to try it with a realistic 13-credit statement and 16 bills. The engine is verified by a headless test asserting every classification on the sample (9 matched, 1 part, 1 probable, 3 exceptions, ₹1,30,604 attention) plus the PDF line-parser and session persistence paths (19 assertions total). **Sessions persist** to localStorage after a run — reloading shows a Resume banner so a distributor never re-imports — and "Start over" clears everything. Nothing ever leaves the device.
